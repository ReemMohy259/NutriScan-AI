...
public class ScanSearchSyncListener {
    ...
    public void insertScanDocumentToElasticSearch(
            ScanStatusChangedEvent event,
            Message message) {
        ...
        try {
            ...
            log.warn("Failed indexing scan {} (attempt {}/{})",
                    event.scanId(),
                    retries,
                    MAX_RETRIES,
                    ex
                ...
                rabbitTemplate.convertAndSend(
                        ...
                        m -> {
                            m.getMessageProperties()
                                    .setHeader(RETRY_HEADER, retries);
                            return m;
                        });

                log.error("Scan {} moved to DLQ after {} retries",
                        event.scanId(),
                        retries);
            ...
            rabbitTemplate.convertAndSend(
                    ...
                    m -> {
                        m.getMessageProperties()
                                .setHeader(RETRY_HEADER, retries + 1);
                        return m;
                    });
        }
    }
